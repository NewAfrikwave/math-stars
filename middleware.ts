...
async function valid(value: string | undefined) {
  ...
  if (scope !== "family" || !expiresRaw || !supplied || Number(expiresRaw) <= Math.floor(Date.now() / 1000)) return false;
  const key = await crypto.subtle.importKey("raw", new TextEncoder().encode(secret), { name: "HMAC", hash: "SHA-256" }, false, ["sign"]);
  const bytes = await crypto.subtle.sign("HMAC", key, new TextEncoder().encode(`${scope}.${expiresRaw}`));
