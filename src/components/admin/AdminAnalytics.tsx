...
function Overview({ data, onNavigate, onShowOutcomes }: { data: AnalyticsData; onNavigate: (tab: AdminTab) => void; onShowOutcomes: () => void }) {
  const adoption = data.totalFamilies > 0 ? Math.round((data.installedDevices / data.totalFamilies) * 100) : 0;
  return (
...
function Devices({ data }: { data: AnalyticsData }) {
  ...
  return <div className="space-y-4"><div className="grid gap-3 sm:grid-cols-3"><SmallKpi label="Known devices" value={data.totalDevices} icon={<MonitorSmartphone />} /><SmallKpi label="Installed apps" value={data.installedDevices} icon={<Download />} /><SmallKpi label="Install adoption" value={`${data.totalFamilies ? Math.round((data.installedDevices / data.totalFamilies) * 100) : 0}%`} icon={<Smartphone />} /></div><div className="grid gap-3 xl:grid-cols-2"><MetricCard><h2 className="font-black">Device mix</h2><div className="mt-4 h-72"><ResponsiveContainer width="100%" height="100%"><BarChart data={chartData} layout="vertical" margin={{ left: 20, right: 15 }}><CartesianGrid stroke="#ece9e3" horizontal={false} /><XAxis type="number" allowDecimals={false} tick={{ fontSize: 11, fill: "#677188" }} /><YAxis type="category" dataKey="label" width={82} tick={{ fontSize: 11, fill: "#39435a" }} /><Tooltip contentStyle={tooltipStyle} /><Bar dataKey="count" fill="#734deb" radius={[0, 7, 7, 0]} /></BarChart></ResponsiveContainer></div></MetricCard><MetricCard><h2 className="font-black">Platforms</h2><div className="mt-5 grid gap-3 sm:grid-cols-2">{data.platformMix.map((platform) => <div key={platform.name} className="rounded-xl border border-[#e5e2dc] bg-[#fbfaf7] p-4"><p className="text-sm font-semibold text-[#667188]">{platform.name}</p><p className="mt-1 text-3xl font-black">{platform.count}</p></div>)}</div></MetricCard></div><MetricCard><h2 className="mb-4 font-black">Recently active devices</h2><RecentDevices devices={data.recentDevices} /></MetricCard></div>;
}
