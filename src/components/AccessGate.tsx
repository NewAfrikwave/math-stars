...
export function AccessGate({ authenticated, children }: { authenticated: boolean; children: React.ReactNode }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setError(response.status === 429 ? "Too many attempts. Please wait and try again." : "That access code is not correct.");
    } catch {
      ...
      <section id="top" className="relative px-5 pb-12 pt-8 sm:px-8 sm:pt-10">
        <div className="mx-auto w-full max-w-6xl text-center">
          ...
          <form
            id="family-access"
            onSubmit={submit}
            className="relative z-10 mx-auto -mt-3 flex max-w-5xl flex-col items-stretch gap-4 rounded-3xl border border-rose-100 bg-white p-5 text-left shadow-[0_18px_55px_rgba(74,38,21,0.13)] sm:flex-row sm:items-center sm:p-6"
            aria-labelledby="family-login-title"
          >
            <div className="flex min-w-0 items-center gap-3 sm:w-[320px]">
              <span className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-rose-50 text-rose-600">
                <LockKeyhole className="h-6 w-6" aria-hidden="true" />
              </span>
              <div>
                <h2 id="family-login-title" className="font-display text-base font-bold">Enter your family access code</h2>
                <p className="mt-0.5 text-xs leading-5 text-stone-500">Step through the door to your private family space.</p>
              </div>
            </div>
            <label htmlFor="family-code" className="sr-only">Family access code</label>
            <input
              id="family-code"
              type="password"
              ...
              aria-describedby={error ? "family-code-error" : "family-code-help"}
              className="h-12 min-w-0 flex-1 rounded-xl border border-stone-200 bg-[#fffdf8] px-4 text-base outline-none transition focus:border-rose-400 focus:ring-4 focus:ring-rose-100"
            />
            <button
              disabled={loading}
              className="flex h-12 shrink-0 items-center justify-center gap-2 rounded-xl bg-rose-600 px-7 font-bold text-white shadow-sm transition hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {loading ? <Loader2 className="h-5 w-5 animate-spin" aria-label="Signing in" /> : <>Enter Math Stars <ChevronRight className="h-4 w-4" aria-hidden="true" /></>}
            </button>
            {error && <p id="family-code-error" role="alert" className="text-sm font-semibold text-red-600 sm:absolute sm:left-[352px] sm:top-[82px]">{error}</p>}
          </form>
